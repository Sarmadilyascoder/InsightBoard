...
import { buildCountryCatalogueUrl, buildWorldBankUrl, chronologicalSeries, countryAccent, createDashboardExport, formatValue, latestObservation, normalizeBrandColor, normalizeFooterText, parseCustomCsv, percentChange } from "../metrics.js";
...
assert.equal(normalizeBrandColor("#1a2B3c", "#000000"), "#1A2B3C", "brand color normalization accepts six-digit hex colors");
assert.equal(normalizeBrandColor("blue", "#000000"), "#000000", "brand color normalization falls back for invalid colors");
assert.equal(normalizeFooterText("  hello\n  @  example.com  "), "hello @ example.com", "footer text collapses whitespace for stable PDF rendering");
assert.equal(normalizeFooterText("123456", 4), "1234", "footer text respects its visible length limit");
